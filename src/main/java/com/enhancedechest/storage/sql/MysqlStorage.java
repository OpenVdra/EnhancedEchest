...
public final class MysqlStorage extends AbstractSqlStorage {
    ...
    // written on join and whenever the name changes. username is VARCHAR(48) (nullable — a row can
    // exist, e.g. from an offline admin resize, before any name is recorded) to comfortably fit Java
    // (16), Bedrock/Floodgate-prefixed names and the like. last_online is epoch-ms of the last
    // join/quit, 0 when never recorded; it decides which offline players tab-completion offers.
    ...
    public MysqlStorage(PluginConfig config) {
        super(buildConfig(config), config.getTablePrefix(), UpsertSyntax.ON_DUPLICATE_KEY,
