...
- **`<size>`**: slot count, a multiple of 9 from 9 to 54.
- **Largest wins**: if a player somehow holds `...27` and `...54`, they get 54. (This differs from the *additional* chests, which stack — a player only has one base chest, so there is one size to pick.)
...
- **Permission-managed while set**: while a `default_size` permission applies to a player, their base chest behaves like a permission-granted chest for admins — `/ee resize` refuses to change it (its size is owned by the permission). This holds even for offline players.
