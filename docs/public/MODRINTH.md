...
- **Migration**: imports vanilla ender chest data automatically on join, or on demand with `/ee migrate run <player>` / `all`. Each player is migrated only once.
...
- `/ee migrate run <player> | all`: import vanilla ender chest data
