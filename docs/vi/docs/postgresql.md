...
**Phù hợp cho:** các trường hợp giống [MySQL / MariaDB](/vi/docs/mysql-mariadb), máy chủ lớn và mạng cần [Liên Máy Chủ](/vi/docs/cross-server), nếu Postgres đã là lựa chọn cơ sở dữ liệu sẵn có của bạn.
...
- Đặt `ssl` thành `require` hoặc `verify-full` để mã hóa kết nối, xem trang [SSL / TLS](/vi/docs/ssl-tls).
