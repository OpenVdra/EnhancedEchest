...
Mở biểu mẫu để sao chép toàn bộ dữ liệu từ một backend cơ sở dữ liệu cũ vào backend đang dùng (ví dụ SQLite → MySQL). Cơ sở dữ liệu đang dùng phải trống và không được có người chơi nào khác trực tuyến. Xem [Chuyển Đổi Backend](/vi/docs/switching-backends) để biết hướng dẫn đầy đủ.
