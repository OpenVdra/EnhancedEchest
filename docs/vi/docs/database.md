...
**Việc nhiều máy chủ dùng chung một cơ sở dữ liệu không được hỗ trợ.** Mỗi máy chủ giữ bản sao riêng dữ liệu của người chơi đang trực tuyến trong bộ nhớ và tự ghi về cơ sở dữ liệu theo lịch của nó, nên hai máy chủ trỏ vào cùng một cơ sở dữ liệu (ví dụ sau một proxy với việc chuyển server nhanh) có thể ghi đè thay đổi của nhau và làm mất hoặc nhân đôi vật phẩm.

Hãy chạy mỗi máy chủ với cơ sở dữ liệu riêng (hoặc tệp SQLite riêng). Tính năng rương Ender liên máy chủ tạm thời sẽ không được hỗ trợ.
